...
function answerTokens(answer, unit) {
  ...
  const words = [...(unit.words || []), ...(unit.vocab || []).map((entry) => entry.word)]
    .filter(Boolean)
...
function asDragQuestion(unit, item, title) {
  const tokens = item.tokens?.length ? item.tokens : answerTokens(item.answer, unit);
...
function dragTitleForTask(taskType) {
  ...
  if (taskType === "จำคำสำคัญ") return "ลากคำสำคัญ";
...
function derivedDragQuestions(unit) {
  ...
  const fromVocab = (unit.vocab || [])
    .filter((entry) => isChineseText(entry.word))
    .map((entry) => asDragQuestion(unit, {
      prompt: `ลากคำจีนให้ตรงกับความหมาย: ${entry.thai}`,
      answer: entry.word,
    }, "ลากคำสำคัญ"));

  return [...fromExamples, ...fromVocab].filter(Boolean);
}
...
function buildMixedQuestions(unit) {
  ...
  unit.tasks.forEach((task) => {
    ...
    addBucket(
      ...
      bucketId,
      title,
      mergeUniqueQuestions(task.prompts.map((item) => asDragQuestion(unit, item, title)).filter(Boolean))
    );
  });
